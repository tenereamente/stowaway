...
To change the postgres password or otherwise edit encrypted secrets, `cd chef` and `knife solo data bag edit custom secrets`.
...
7. Manually configure DNS to point to the server

8. TODO: SSL certificates
...
TODO: fix deployment of production environment with secure cookie key and DB passwords

1. Run `cap development deploy`. Repeat as needed to try new versions of the application.
3. After a deploy, visit the [dev version of StowAway](http://localhost:3000). Register for an account.
